...
```bash
python src/train_tcn.py --data_dir data --gestures_yaml gestures.yaml --epochs 40 --batch_size 32 --lr 1e-3 --model_size small --use_class_weights --augment
```

> [!NOTE]
> El dataset se divide automáticamente en **70% Entrenamiento**, **15% Validación** y **15% Test**, manteniendo la proporción de clases (estratificado). Las muestras se descubren directamente desde `data/raw/`.
